...
function and random generation for the Ex-Wald distribution with
parameter \\\mu\\, \\\sigma\\ and \\\nu\\.
...
`dExWALD` gives the density, `pExWALD` gives the distribution function,
`qExWALD` gives the quantile function, `rExWALD` generates random
deviates.
