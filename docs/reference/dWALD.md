...
function and random generation for the Wald distribution with parameter
\\\mu\\ and \\\sigma\\.
...
`dWALD` gives the density, `pWALD` gives the distribution function,
`qWALD` gives the quantile function, `rWALD` generates random deviates.
...
\\f(x \|\mu, \sigma)=\frac{\sigma}{\sqrt{2 \pi x^3}} \exp
\left\[-\frac{(\sigma-\mu x)^2}{2x}\right \],\\
...
[WALD](http://fhernanb.github.io/RelDists/reference/WALD.md)
