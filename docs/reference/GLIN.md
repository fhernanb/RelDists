...
The Generalized Lindley Type II (GLIN) family for fitting positive
continuous lifetime data within the GAMLSS framework.

## Usage

``` r
GLIN(mu.link = "log", sigma.link = "log")
```
...
Returns a gamlss.family object which can be used to fit a GLIN
distribution in the
...
The GLIN distribution has been proposed for modeling lifetime and
